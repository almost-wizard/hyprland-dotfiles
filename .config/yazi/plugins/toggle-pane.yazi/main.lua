...
local function entry(st, job)
	...
	-- Keep the user's configured pane ratio as the restore target.
	st.parent = st.parent or R.parent
	st.current = st.current or R.current
	st.preview = st.preview or R.preview

	-- Toggle the requested pane between normal, hidden, or maximized.
	...
	-- Override the layout until reset/restored.
	if not st.old then
		st.old = Tab.layout
